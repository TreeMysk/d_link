<?php
	...
	class functions {
		...
		function doPages($page_size, $thepage, $query_string, $total=0, $keyword) {
			...
			if ($current == 1) {
				echo '<li class="disabled"><a>Atgal</a></li>';
			} else {
				$i = $current - 1;
				echo '<li><a href="'.$thepage.'?page='.$i.$query.'&keyword='.$keyword.'" rel="nofollow" title="go to page '.$i.'">Atgal</a></li>';
				//echo '<p>...</p>&nbsp;';
			}
				//<button>'.$i.'</button>
			if ($start > 1) {
			...
			if ($current < $total_pages) {
				$i = $current + 1;
				//echo '<p>...</p>&nbsp;';
				echo '<li><a href="'.$thepage.'?page='.$i.$query.'&keyword='.$keyword.'" rel="nofollow" title="go to page '.$i.'">Toliau</a></li>';
			} else {
				echo '<li class="disabled"><a>Toliau</a></li>';
			}
			...
			if ($total != 0) {
				...
				echo '<br><div class="pull-right">( iš viso '.$total.' )</div></div>';
			} else {
				echo '</div>';
			};
